...
class NYUv2(Dataset):
    ...
    label_colors = dict(zip(range(13), colors))

    def __init__(self, root_path, split='', transforms=None, num_tasks=2):
    ...
    def __getitem__(self, index):
        ...
        if normal is not None:
            return image.float(), segmt.long(), depth.float(), normal.float()
    ...
    def _random_scale_crop(self, image, segmt, depth, normal):
        ...
        if normal is not None:
            normal_ = F.interpolate(normal[None, :, i:i + h, j:h + w], size=(height, width), mode='bilinear', align_corners=True).squeeze(0)
        else:
            normal_ = None
